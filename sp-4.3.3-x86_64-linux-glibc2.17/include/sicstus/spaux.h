...
typedef int (SPCDECL tSP_set_memalloc_hooks) PROTOTYPE((int hints, SP_InitAllocHook * init_alloc_hook, SP_DeinitAllocHook * deinit_alloc_hook, SP_AllocHook * alloc_hook, SP_FreeHook * free_hook, void * cookie));
...
typedef struct DISPATCH_TABLE_STRUCT_SICSTUS_H {
   ...
   const char *nSP_set_memalloc_hooks; tSP_set_memalloc_hooks *pSP_set_memalloc_hooks; /* API #77 */
...
extern tSP_codes_to_multi_bytes SP_codes_to_multi_bytes;
   ...
   #define SP_set_memalloc_hooks (GetSICStusAPIProcInit(API_SICSTUS_H,pSP_set_memalloc_hooks,nSP_set_memalloc_hooks,"SP_set_memalloc_hooks"))
...
typedef TAGGED (SPCDECL tdelete_attributes) PROTOTYPE((TAGGED, struct mod_def *, int live));
...
#define SP_API_SUPPORT_H_NUMBER_OF_ENTRIES 41
#if (INCLUDED_FROM_RUNTIME || ECLIPSE_CDT) /* No indirection within SP runtime (or when indexing from CDT) */
