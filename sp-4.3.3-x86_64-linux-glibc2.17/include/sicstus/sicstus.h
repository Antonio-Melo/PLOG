...
/* [PM] 4.0.5
   ...
   Call SP_set_memalloc_hooks(SP_SET_MEMALLOC_HOOKS_HINT_USE_MALLOC,
   0,0,0,0,0) to tell SICStus memory manager to use malloc() et al for
   everything. Useful with valgrind. */
#define SP_SET_MEMALLOC_HOOKS_HINT_USE_MALLOC   0x0001
/* [PM] 4.2.1 undocumented */
#define SP_SET_MEMALLOC_HOOKS_HINT_PARAM_COOKIE 0x0002
#define SP_SET_MEMALLOC_HOOKS_HINT_RESERVED_2   0x0004
#define SP_SET_MEMALLOC_HOOKS_HINT_RESERVED_3   0x0008
#define SP_SET_MEMALLOC_HOOKS_HINT_RESERVED_4   0x0010

union sp_t_memalloc_cookie_param_ { size_t size; void *ptr; };
typedef struct sp_t_memalloc_cookie_ sp_t_memalloc_cookie;
struct sp_t_memalloc_cookie_ {
  size_t size;                  /* sizeof(sp_t_memalloc_cookie) */
/* The allocated memory is generic. */
#define SP_MEMALLOC_COOKIE_FLAG_GENERIC            0x0001
/* The allocated memory will be used for trail+choice stacks. */
#define SP_MEMALLOC_COOKIE_FLAG_STACK_TRAIL_CHOICE 0x0002
/* The allocated memory will be used for global+local stacks. */
#define SP_MEMALLOC_COOKIE_FLAG_STACK_GLOBAL_LOCAL 0x0004
/* The allocated memory must have executable permissions. */
#define SP_MEMALLOC_COOKIE_FLAG_EXECUTABLE         0x0008
/* The allocated memory should (optionally) be allocated using huge pages. */
#define SP_MEMALLOC_COOKIE_FLAG_HUGETBL            0x0010
/* Not yet used */
#define SP_MEMALLOC_COOKIE_FLAG_DRY_RUN            0x0020
  spio_t_bits flags;
  void *cookie;        /* user code can store here. Initially NULL. */
  union sp_t_memalloc_cookie_param_ param1;
};
...
/* [PM] WinCE sane interface to _environ. Also works on WinCE that
   ...
   SP runtime should use spio_set_system_property and sp_getenv_internal
   instead.
*/
