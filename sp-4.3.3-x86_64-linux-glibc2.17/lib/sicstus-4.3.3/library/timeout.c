...
#if !defined(MINIMUM_TIMEOUT_FACTOR)
/* [PM] 4.3.3 Use a larger minimum timeout. This prevents timeout from
   things like time_out(true,1,Res). */
#define MINIMUM_TIMEOUT_FACTOR 10
#endif /* !defined(MINIMUM_TIMEOUT_FACTOR) */
#endif /* SP_WIN32 */
...
#define dbg_fprintf(ARGS) do {if (SHOULD_DBG_PRINTF) {fprintf(stderr, "\n** DBGc: %s:%d",__FILE__,(int)__LINE__); fprintf ARGS; fprintf(stderr, "\n"); fflush(stderr);} } while (0)
#define dbg_printf(ARGS) do {if (SHOULD_DBG_PRINTF) {printf("\n** DBGc:%s:%d",__FILE__,(int)__LINE__); printf ARGS; fflush(stdout);}} while (0)
#else  /* !DEBUG_TIMEOUT */
      ...
      {
        {
          ...
          { /* [PM] 3.10.0b1 See to_start_timer_a */
            SP_integer timer_resolution = (TimerUnitsPerSecond + (local.clocks_per_second - 1))/local.clocks_per_second;
	    SP_integer minimum_timeout = timer_resolution + 1;

#if defined(MINIMUM_TIMEOUT_FACTOR)
	minimum_timeout *= MINIMUM_TIMEOUT_FACTOR;
#endif /* defined(MINIMUM_TIMEOUT_FACTOR) */

            if (time < minimum_timeout)
              {
                dbg_printf(("DBGc: alarm_h: Adjusting original timeout %" SPRIdINTEGER "ms to %" SPRIdINTEGER "ms",
                             (SP_integer)time, (SP_integer)minimum_timeout));
                time = minimum_timeout;
              }
          }
#endif /* PREVENT_IMMEDIATE_TIMEOUT */
...
#endif	/* !defined(USE_CreateThread) */
#endif	/* WINCE */
...
{
  char *value = NULL; /* Needs cleanup */
  (void) alarm_clock_off(NULL);
  (void) SP_register_atom(local.atom_off = SP_atom_from_string("off"));
  if (local.usertime_method == 0) {
    value = SP_getenv("SP_WIN32_TIMEOUT_WALLTIME");
    if (value != NULL && strcmp("yes",value) == 0) {

#if DEBUG_TIMEOUT
      /* not dbg_fprintf */
      fprintf(stderr, "DBG: to_reinit() will use old GetTickCount()-based method (SP_WIN32_TIMEOUT_WALLTIME=yes)\n");fflush(stderr);
#if defined(MINIMUM_TIMEOUT_FACTOR)
      fprintf(stderr, "DBG: to_reinit() MINIMUM_TIMEOUT_FACTOR=%d\n",(int)(MINIMUM_TIMEOUT_FACTOR));fflush(stderr);
#endif /* defined(MINIMUM_TIMEOUT_FACTOR) */
#endif/* DEBUG_TIMEOUT */

      /* [PM] 4.3.3 Use the legacy method, based on GetTickCount(). */
      local.usertime_method = 2;
    }
  }
...
#if DEBUG_TIMEOUT
    /* not dbg_fprintf */
    fprintf(stderr, "DBG: sp_GetTime1(%" SPRIu64 ", %" SPRIu64 ", %" SPRIu64 ")\n",
	    (SP_uint64)kernelTime, (SP_uint64)userTime, (SP_uint64)threadStartTime
	    );fflush(stderr);
#endif/* DEBUG_TIMEOUT */
...
{
  SP_integer res = win32_usertime();

  dbg_fprintf((stderr, "DBG: myGetTickCount()=%" SPRIu64 "\n",
	       (SP_uint64)res
	       ));
...
#if defined(MINIMUM_TIMEOUT_FACTOR)
	/* [PM] 4.3.3 an experiment for working around the poor behavior on Windows. */
	minimum_timeout *= MINIMUM_TIMEOUT_FACTOR;
#endif /* defined(MINIMUM_TIMEOUT_FACTOR) */
